...
export function canAccess(project: Project, email: string, role: UserRole): boolean {
  ...
  return role === "ADMIN"
    || project.ownerEmail.toLowerCase() === normalizedEmail
    || project.memberEmails.some((memberEmail) => memberEmail.toLowerCase() === normalizedEmail);
}
...
const actionRoles: Record<ProjectAction, ProjectRole[]> = {
  EDIT_PROJECT: ["PROJECT_MANAGER"],
  UPDATE_CHECKLIST: ["PROJECT_MANAGER", "REVIEWER"],
  REVIEW_AI: ["PROJECT_MANAGER", "REVIEWER"],
  GENERATE_REPORT: ["PROJECT_MANAGER", "DECISION_MAKER"],
  MANAGE_RISKS: ["PROJECT_MANAGER", "REVIEWER"],
  MANAGE_TASKS: ["PROJECT_MANAGER", "REVIEWER"],
  EDIT_OPERATIONS: ["PROJECT_MANAGER"],
  DECIDE_STAGE: ["PROJECT_MANAGER", "DECISION_MAKER"],
  MANAGE_EXPERT_ASSIGNMENTS: ["PROJECT_MANAGER", "REVIEWER"],
  MANAGE_MEMBERS: ["PROJECT_MANAGER"],
};

export function authorizeProject(project: Project, email: string, role: UserRole, action: ProjectAction): boolean {
  if (!canAccess(project, email, role)) return false;
  if (role === "ADMIN" || project.ownerEmail.toLowerCase() === email.toLowerCase()) return true;
  return actionRoles[action].includes(projectRoleOf(project, email));
}

function canManageProject(project: Project, email: string, role: UserRole): boolean {
  return authorizeProject(project, email, role, "MANAGE_RISKS");
}
...
export async function assignProjectMember(input: {
  ...
  await mutate(async (data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.actorEmail, input.actorRole, "MANAGE_MEMBERS")) {
...
export async function removeProjectMember(input: { projectId: string; email: string; actorEmail: string; actorRole: UserRole }): Promise<void> {
  await mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.actorEmail, input.actorRole, "MANAGE_MEMBERS")) {
      throw new Error("无权管理该项目成员。");
...
export async function updateProject(input: {
  ...
  return mutate((data) => {
    ...
    if (!project || !authorizeProject(project, input.email, input.role, "EDIT_PROJECT")) return null;
...
export async function updateChecklistItem(input: {
  ...
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.email, input.role, "UPDATE_CHECKLIST")) return false;
...
export async function updateChecklistAIAssessment(input: {
  ...
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    const checklistItem = project?.checklist.find((item) => item.code === input.checklistCode);
    if (!project || !checklistItem?.aiAssessment || !authorizeProject(project, input.email, input.role, "REVIEW_AI")) return false;
    const now = new Date().toISOString();
...
export async function finalizeAIReviewWithoutFindings(input: { projectId: string; reviewId: string; email: string; role: UserRole; note: string }): Promise<AIReview | null> {
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    const review = data.aiReviews.find((item) => item.id === input.reviewId && item.projectId === input.projectId);
    if (!project || !review || !authorizeProject(project, input.email, input.role, "REVIEW_AI") || review.status !== "REVIEW_REQUIRED" || !review.output || review.output.findings.length !== 0) return null;
    const now = new Date().toISOString();
...
export async function decideAIReviewFinding(input: {
  ...
  return mutate((data) => {
    ...
    if (!project || !review || !output || !finding || !authorizeProject(project, input.email, input.role, "REVIEW_AI") || review.status !== "REVIEW_REQUIRED" || finding.status !== "PENDING") return null;
    const now = new Date().toISOString();
...
export async function createReport(input: { projectId: string; email: string; role: UserRole; outcome: ReportDecision; rationale: string }): Promise<ProjectReport | null> {
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.email, input.role, "GENERATE_REPORT")) return null;
...
export async function softDeleteDocument(projectId: string, documentId: string, email: string, role: UserRole): Promise<ProjectDocument | null> {
  return mutate((data) => {
    ...
    if (!project || !document || !authorizeProject(project, email, role, "UPDATE_CHECKLIST")) return null;
...
export async function createRisk(input: {
  ...
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.email, input.role, "MANAGE_RISKS")) return null;
    const now = new Date().toISOString();
    const risk: Risk = {
...
export async function updateProjectProfile(input: { projectId: string; email: string; role: UserRole; profile: ProjectProfile }): Promise<Project | null> {
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.email, input.role, "EDIT_OPERATIONS")) return null;
...
export async function deleteCandidateVenue(input: { projectId: string; venueId: string; email: string; role: UserRole }): Promise<boolean> {
  return mutate((data) => {
    ...
    if (!project || !venue || !authorizeProject(project, input.email, input.role, "EDIT_OPERATIONS") || venue.isPrimary) return false;
...
export async function updateStageDecision(input: { projectId: string; stageId: string; email: string; role: UserRole; status: ProjectStage["status"]; decision: ProjectStage["decision"]; conditions: string; note: string }): Promise<ProjectStage | null> {
  return mutate((data) => {
    ...
    if (!project || !stage || !authorizeProject(project, input.email, input.role, "DECIDE_STAGE")) return null;
...
export async function updateDecisionGate(input: { projectId: string; gateId: string; email: string; role: UserRole; decision: DecisionGate["decision"]; rationale: string }): Promise<DecisionGate | null> {
  return mutate((data) => {
    ...
    if (!project || !gate || !authorizeProject(project, input.email, input.role, "DECIDE_STAGE")) return null;
...
export async function createGeneralTask(input: { projectId: string; email: string; role: UserRole; title: string; assigneeEmail: string; dueDate: string; priority: NonNullable<RemediationTask["priority"]> }): Promise<RemediationTask | null> {
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.email, input.role, "MANAGE_TASKS")) return null;
    const now = new Date().toISOString();
...
export async function createExpertAssignment(input: Omit<ExpertAssignment, "id" | "createdAt" | "updatedAt" | "status" | "opinion" | "attachmentDocumentIds" | "createdBy"> & { email: string; role: UserRole }): Promise<ExpertAssignment | null> {
  return mutate((data) => {
    const project = data.projects.find((item) => item.id === input.projectId);
    if (!project || !authorizeProject(project, input.email, input.role, "MANAGE_EXPERT_ASSIGNMENTS")) return null;
    const now = new Date().toISOString();
...
export async function updateExpertAssignment(input: { projectId: string; assignmentId: string; email: string; role: UserRole; status: ExpertAssignment["status"]; opinion: string }): Promise<ExpertAssignment | null> {
  return mutate((data) => {
    ...
    const canManage = project ? authorizeProject(project, input.email, input.role, "MANAGE_EXPERT_ASSIGNMENTS") : false;
    const isAssignedExpert = assignment.expertEmail.toLowerCase() === input.email.toLowerCase() && projectRoleOf(project, input.email) === "EXPERT";
    if (!canManage && !isAssignedExpert) return null;
    if (isAssignedExpert && !canManage && !["ACCEPTED", "MORE_INFO", "DELIVERED"].includes(input.status)) return null;
...
export async function updateProjectMemberRole(input: { projectId: string; memberEmail: string; projectRole: ProjectRole; actorEmail: string; actorRole: UserRole }): Promise<boolean> {
  return mutate((data) => { const project = data.projects.find((item) => item.id === input.projectId); if (!project || !authorizeProject(project, input.actorEmail, input.actorRole, "MANAGE_MEMBERS")) return false;
    const memberEmail = input.memberEmail.toLowerCase();
    if (!project.memberEmails.some((email) => email.toLowerCase() === memberEmail)) return false;
    project.memberRoles ??= {}; project.memberRoles[memberEmail] = input.projectRole; audit(data, { projectId: project.id, actorEmail: input.actorEmail, action: "PROJECT_ROLE_UPDATED", entityType: "USER", entityId: memberEmail }); return true; });
}
