...
const unauthorizedMember = await store.createManagedUser({ actorEmail: adminEmail, actorRole: "ADMIN", displayName: "未授权验收成员", email: `security-outsider-${Date.now()}@example.com`, password: "security-smoke-password-2026", role: "MEMBER" });
await store.assignProjectMember({
  projectId: "demo-shanghai-pudong-a",
  displayName: securityMember.displayName,
  email: securityMember.email,
  temporaryPassword: "security-smoke-password-2026",
  actorEmail: adminEmail,
  actorRole: "ADMIN",
});
const fileStorageModule = await import("../src/lib/file-storage.ts");
const fileStorage = fileStorageModule.saveValidatedFile ? fileStorageModule : fileStorageModule.default;
const storedSensitiveFile = await fileStorage.saveValidatedFile(new File(
  [Buffer.from("%PDF-1.4\n% security smoke fixture\n")],
  "security-smoke-sensitive.pdf",
  { type: "application/pdf" },
));
const sensitiveDocument = await store.addDocument({
  projectId: "demo-shanghai-pudong-a",
  email: securityMember.email,
  role: "MEMBER",
  category: "OTHER",
  fileName: "security-smoke-sensitive.pdf",
  sensitive: true,
  ...storedSensitiveFile,
});
if (!sensitiveDocument) throw new Error("无法创建敏感文件安全测试数据。");
...
try {
  ...
  const outsider = cookie(unauthorizedMember.email, "MEMBER");
  const projectMember = cookie(securityMember.email, "MEMBER");
  ...
  const sensitiveUrl = `${origin}/api/projects/demo-shanghai-pudong-a/documents/${sensitiveDocument.id}`;
  assert.equal((await fetch(sensitiveUrl, { headers: { cookie: projectMember } })).status, 403);
  assert.equal((await fetch(`${sensitiveUrl}?download=1`, { headers: { cookie: projectMember } })).status, 403);
  assert.equal((await fetch(sensitiveUrl, { headers: { cookie: admin } })).status, 200);
  ...
  console.log(JSON.stringify({ passed: 18 + pdfChecks, failed: 0, p95Ms: Number(p95Ms.toFixed(1)), origin }));
} finally {
  server.kill("SIGTERM");
  await new Promise((resolve) => server.once("exit", resolve));
  await rm(path.join(".data", "uploads", storedSensitiveFile.storageKey), { force: true });
